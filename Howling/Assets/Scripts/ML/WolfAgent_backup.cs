...
public class WolfAgent : Agent
{
    ...
    public void Eat()
    {
        if (CanEat)
        {
            ...
            if (adj != null)
            {
                ...
                AddReward((Hungry - oldHungy) * 0.01f);
    ...
    bool CanDig
    {
        get
        {
            var colliders = Physics.OverlapSphere(transform.position, targetRange * 10, colliderLayerMask);
    ...
    public void DecreaseHP(float atk)
    {
        Hp -= atk;
        AddReward(-0.01f * atk / 2);
    }
}